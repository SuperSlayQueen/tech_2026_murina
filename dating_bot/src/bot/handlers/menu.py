...
from src.backend.models import User, Profile, ProfilePhoto, Like, Match, Rating, Event
...
# Хранилище текущих просматриваемых профилей и очереди анкет
user_queues = {}  # {telegram_id: [список анкет]}
...
async def back_to_menu(callback: CallbackQuery):
    ...
    telegram_id = callback.from_user.id
    if telegram_id in user_queues:
        del user_queues[telegram_id]
...
async def search(callback: CallbackQuery, session: AsyncSession):
    ...
    # Проверяем, есть ли очередь анкет для этого пользователя
    if telegram_id in user_queues and len(user_queues[telegram_id]) > 0:
        next_profile = user_queues[telegram_id].pop(0)
    ...
    if len(ranked_profiles) > 1:
        user_queues[telegram_id] = ranked_profiles[1:]
    else:
        user_queues[telegram_id] = []
...
async def like_profile(callback: CallbackQuery, session: AsyncSession):
    ...
    if existing_like.scalar_one_or_none() is None:
        session.add(Like(from_user_id=current_user.id, to_user_id=target_user_id))
        session.add(
            Event(
                event_type="like",
                payload={"from_user_id": current_user.id, "to_user_id": target_user_id}
            )
        )
    ...
    if reverse_like.scalar_one_or_none():
        ...
        if existing_match.scalar_one_or_none() is None:
            ...
            session.add(
                Event(
                    event_type="match",
                    payload={"user1_id": user1, "user2_id": user2}
                )
            )
            ...
            if current_profile and target_profile and target_user:
                me_link = f'<a href="tg://user?id={current_user.telegram_id}">{current_profile.name}</a>'
                target_link = f'<a href="tg://user?id={target_user.telegram_id}">{target_profile.name}</a>'

                await callback.bot.send_message(
                    chat_id=current_user.telegram_id,
                    text=f"💘 У вас взаимный мэтч с {target_link}! Нажми на имя и напиши первым(ой).",
                    parse_mode="HTML"
                )
                await callback.bot.send_message(
                    chat_id=target_user.telegram_id,
                    text=f"💘 У вас взаимный мэтч с {me_link}! Нажми на имя и напиши первым(ой).",
                    parse_mode="HTML"
                )
